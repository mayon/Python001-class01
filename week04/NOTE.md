...
本周学习内容集中在数据处理这一块，尤其是 Pandas 这个库，有大量的常用方法需要掌握。在处理数据时，有大量跟数据库类似的操作，目前刚接触，用起来不熟练，需要反复 debug。感觉这周的学习内容很适合上一周第二题作业，对抓取回来的数据进行清洗、展示。接下来要把上周第二题补上，以便练习这周学到的各种方法。